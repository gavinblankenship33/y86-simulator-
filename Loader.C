...
void Loader::loadLine(std::string line)
{
	...
	while (line[i] != ' ')
	{
		Memory::getInstance()->putByte(convert(line, i, 2), addr + count, error);
		lastAddress = addr + count;
		count++;
		i += 2;
	}
}
...
{
	return std::stoul(line.substr(start, len), NULL, 16);
}
...
bool Loader::hasErrors(std::string line)
{
   ...
   //   to lastAddress
	...
	int32_t addr = convert(line, ADDRBEGIN, ADDREND);
    	if (addr < lastAddress) return true;
   ...
   //         and addr returned by convert
 	...
 	if (addr + numDBytes > MEMSIZE) return true;	
...
{
	std::string data = line.substr(DATABEGIN, (COMMENT + 1) - DATABEGIN);
...
{
	if (line[ADDRBEGIN - 2] != '0') return true;
    	if (line[ADDRBEGIN - 1] != 'x') return true;

	if (!isxdigit(line[ADDRBEGIN])) return true;
	if (!isxdigit(line[ADDRBEGIN + 1])) return true;
	if (!isxdigit(line[ADDRBEGIN + 2])) return true;
...
{	
	if (filename.find(".yo") == false) return true;
