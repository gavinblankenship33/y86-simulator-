...
{	 
	uint64_t myLong = 0x0000000000000000;
	int i = 0;
	for (i = 0; i < LONGSIZE; i++)
	{	
		myLong = myLong << 8;
		myLong = myLong + bytes[(LONGSIZE - 1) - i];
	}
	return myLong;
}
...
{
	if (srclow < 0 || dstlow < 0 || srclow > 63 || dstlow > 63 ||
 	(length - 1) + srclow > 63 || (length - 1) + dstlow > 63) {
    		return dest;
  	}

  	uint64_t srcBits = getBits(source, srclow, (length - 1) + srclow) << dstlow;
  	uint64_t cleared = clearBits(dest, dstlow, (length - 1) + dstlow);

  	uint64_t result = srcBits | cleared;
...
{
	uint64_t gb = getBits(source, 63, 63);
	return gb;
}
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2)
{
	...
	uint64_t add = op1 + op2;
	uint64_t addSign = sign(add);
	
	return (op1Sign == op2Sign && op2Sign != addSign);
}
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2)
{
	...
	bool result = (op1Sign != op2Sign && subSign == op1Sign);

	//std::cout << "Op 1(" << op1 << ") Sign = " << op1Sign << std::endl;
	//std::cout << "Op 2(" << op2 << ") Sign = " << op2Sign << std::endl;
	//std::cout << "Sub(" << sub << ") Sign = " << subSign << std::endl;
	//std::cout << "Sub OF Result(a != b && b == c) = " << result << std::endl;
